...
export default function Pro() {
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (name === "day" || name === "month") {
      if (/^\d{0,2}$/.test(value)) setFormData({ ...formData, [name]: value });
    } else if (name === "year") {
      if (/^\d{0,4}$/.test(value)) setFormData({ ...formData, [name]: value });
    } else {
    ...
    <div className="flex flex-col items-start justify-start relative text-white h-screen p-10">
      ...
      <div className="flex flex-col mt-14">
        ...
        <div className="mt-6 flex flex-col items-start">
          ...
          {activeTab && (
            <div
              className="fixed left-1/2 top-1/2 transform -translate-x-1/2 -translate-y-1/2 p-6 border-2 border-white rounded-lg backdrop-blur-lg text-white flex flex-col gap-4 bg-gray-900 shadow-xl"
              style={{ width: "500px" }}
            >
              {activeTab === "history" ? (
                ...
                <>
                  ...
                  <button className="px-10 py-2 border-2 border-green-500 text-green-500 font-bold rounded-lg mt-5 bg-transparent w-full hover:bg-green-500 hover:text-white transition-all duration-500">
                    Confirm
                  </button>
                </>
              )}
              <button
                ...
                className="px-4 py-2 mt-4 text-white bg-red-500 rounded-md hover:bg-red-600 transition-all duration-500"
              >
                Close
              </button>
            </div>
          ...
          <Link href="/forgetPassword">
            <button className="px-10 py-2 border-2 border-white text-white font-bold rounded-lg mt-5 bg-transparent w-56 h-12 hover:bg-gray-700 transition-all duration-500">
              Reset Password
            </button>
          </Link>
          <Link href="/login">
